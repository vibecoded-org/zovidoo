...
@Component({ selector: 'app-four-choice', template: `<div class="choices" role="group">@for (option of options; track option) { <button #choiceButton class="choice" [class.correct]="feedback === 'correct' && option === answer" [class.incorrect]="feedback === 'incorrect' && option === selected" [disabled]="disabled" [attr.aria-pressed]="option === selected" (click)="pick(option)">{{ option }} @if (feedback === 'correct' && option === answer) { <span aria-hidden="true">✓</span> }</button> }</div>`, styleUrls: ['./answer-components.scss'], changeDetection: ChangeDetectionStrategy.OnPush, standalone: true })
export class FourChoiceComponent implements AfterViewChecked {
  @Input() options: string[] = []; @Input() disabled = false; @Input() feedback: 'correct' | 'incorrect' | '' = ''; @Input() answer = ''; @Input() selected = ''; @Output() answered = new EventEmitter<string>();
