...
@Component({ selector: 'app-pitch-keyboard', template: `<div class="piano" role="group" [attr.aria-label]="i18n.t('pianoAnswers')">@for (key of whiteKeys; track key) { <button #pianoKey class="white-key" [class.selected]="selected === key" [disabled]="disabled" [attr.aria-pressed]="selected === key" (click)="select(key)" [attr.aria-label]="key">{{ key }}</button> } @for (key of blackKeys; track key.key) { <button #pianoKey class="black-key" [style.left.%]="key.left" [class.selected]="selected === key.key" [disabled]="disabled" [attr.aria-pressed]="selected === key.key" (click)="select(key.key)" [attr.aria-label]="key.key.replace('/', ' / ')">{{ key.key.split('/')[0] }}</button> }</div>`, styleUrls: ['./answer-components.scss'], changeDetection: ChangeDetectionStrategy.OnPush, standalone: true })
export class PitchKeyboardComponent implements AfterViewChecked {
 readonly i18n = inject(TranslationService);
 @Input() disabled = false; @Input() selected = ''; @Output() answered = new EventEmitter<PitchClass>(); whiteKeys = whites; blackKeys = blacks; select(key: PitchClass): void { this.answered.emit(key); }
