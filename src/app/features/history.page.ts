...
export class HistoryPage {
  ...
  label(type: SessionSummary['type']): string { return type === 'quick' ? this.i18n.t('historyQuick') : this.i18n.t(`history${({ note: 'Note', interval: 'Interval', chord: 'Chord', 'chord-symbol': 'ChordSymbol', progression: 'Progression', 'progression-chords': 'ProgressionChords' })[type]}`); }
  ...
  skillLabel(session: SessionSummary): string { if (session.type === 'quick') return this.i18n.t('skillsCovered', { count: Object.keys(session.exerciseBreakdown ?? {}).length }); const skill = this.storage.skillProgress()[session.type]; return this.i18n.t('skillLevel', { level: skill.level, score: skill.score }); }
}
