...
export class StorageService {
  ...
  private restore(): AppData {
    try {
      ...
      if (raw) localStorage.setItem(CORRUPT_STORAGE_KEY, raw);
      return emptyData();
    }
  }
  private migrate(data: Partial<AppData>): AppData {
    const sessions = Array.isArray(data.sessions) ? data.sessions.filter(this.validSession).map(session => this.normalizeSession(session)).slice(0, 2000) : [];
  ...
  private validSession = (session: unknown): session is SessionSummary => !!session && typeof session === 'object' && typeof (session as SessionSummary).id === 'string' && (validExerciseType((session as SessionSummary).type) || (session as SessionSummary).type === 'quick') && validDate((session as SessionSummary).startedAt) && validDate((session as SessionSummary).completedAt) && finite((session as SessionSummary).durationMs) && (session as SessionSummary).durationMs >= 0 && nonNegativeInteger((session as SessionSummary).questions) && nonNegativeInteger((session as SessionSummary).correctAnswers) && nonNegativeInteger((session as SessionSummary).incorrectAnswers) && nonNegativeInteger((session as SessionSummary).firstAttemptCorrect) && (session as SessionSummary).correctAnswers <= (session as SessionSummary).questions && (session as SessionSummary).firstAttemptCorrect <= (session as SessionSummary).questions && bounded((session as SessionSummary).difficulty, 1, 5, 0) > 0 && this.validExerciseBreakdown((session as SessionSummary).exerciseBreakdown) && this.validSkillResults((session as SessionSummary).skillResults);
