...
export class AudioEngineService {
  ...
  async initialize(): Promise<void> {
    await Tone.start();
    this.ensureSynth(this.storage.settings().instrument);
    this.ready = true;
  }
  ...
  dispose(): void { this.cancelPlayback(); this.ready = false; }
  private ensureSynth(instrument: Instrument): void {
    const playableInstrument = this.resolveInstrument(instrument);
  ...
  private resolveInstrument(instrument: Instrument): PlayableInstrument {
