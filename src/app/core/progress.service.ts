...
import { EXERCISE_TYPES, ExerciseType, SessionSummary, SkillProgressMap } from './models';
import { StorageService } from './storage.service';

export interface ProgressSnapshot { totalSessions: number; totalQuestions: number; correct: number; incorrect: number; accuracy: number; totalMinutes: number; todayQuestions: number; todayAccuracy: number; currentStreak: number; longestStreak: number; weeklyDays: number; monthlyDays: number; activeDays: Set<string>; byType: Record<ExerciseType, number>; skills: SkillProgressMap; }
...
export class ProgressService {
  ...
  readonly snapshot = computed<ProgressSnapshot>(() => this.aggregate(this.storage.sessions(), this.storage.skillProgress()));
  dateKey = dateKey;
  private aggregate(sessions: SessionSummary[], skills: SkillProgressMap): ProgressSnapshot {
    ...
    const correct = sessions.reduce((sum, s) => sum + s.firstAttemptCorrect, 0);
