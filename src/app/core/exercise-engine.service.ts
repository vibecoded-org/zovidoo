...
export class ExerciseEngineService {
  constructor(private readonly theory: MusicTheoryService, private readonly i18n?: TranslationService) {}
  generate(type: ExerciseType, difficulty: Difficulty = 1, random = Math.random, options: ExerciseGenerationOptions = {}): ExerciseQuestion {
    switch (type) { case 'note': return this.note(random, difficulty); case 'interval': return this.interval(random, difficulty); case 'chord': return this.chordQuality(random); case 'chord-symbol': return this.chordSymbol(random, difficulty); case 'progression-chords': return this.progressionChords(random, difficulty, options); case 'progression': return this.progression(random, difficulty, options); }
  }
